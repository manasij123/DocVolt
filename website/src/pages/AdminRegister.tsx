  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post("/auth/register", {
        name: name.trim(), email: email.trim(), password, role: "admin",
      });
        ...
        <div className="card card-md">
          ...
          <form onSubmit={submit}>
            <div className="field"><label>Full name</label>
              <div className="input-with-icon"><span className="field-prefix">👤</span>
                <input className="input" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name or org" required />
              </div>
            </div>
            <div className="field"><label>Email <span className="muted" style={{ fontSize: 11, fontWeight: 600 }}>(clients will use this to connect)</span></label>
              <div className="input-with-icon"><span className="field-prefix">✉️</span>
                <input className="input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@org.com" required autoComplete="email" />
              </div>
            </div>
            ...
            <div className="hint">Already have an account? <Link to="/admin/login" style={{ color: "#1A73E8", fontWeight: 700 }}>Sign in</Link></div>
          </form>
        </div>
      </div>
    </div>
