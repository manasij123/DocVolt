...
export async function suggestCategory(client_id: string, filename: string, _token?: string): Promise<CategorySuggestion> {
  const r = await api.get<CategorySuggestion>("/categories/suggest", { params: { client_id, filename } });
  return r.data;
}
