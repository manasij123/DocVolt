...
agent_communication:
    ...
    - agent: "main"
      message: "Mobile P0 Blocker FIXES complete (June 2026 session): (1) **Backend sort bug** — `_ensure_default_categories` was crashing with `TypeError: '<' not supported between instances of 'datetime.datetime' and 'str'` because some legacy category rows had datetime `created_at` while newer seeds had ISO strings. Fixed by wrapping in `str(...)` in `/app/backend/server.py` line 373. This was causing GET /api/categories to return 500 intermittently. (2) **useLocalSearchParams bug in nested tab screens** — In Expo Router web mode, `useLocalSearchParams` returns `undefined` inside child screens of a nested `[clientId]/_layout.tsx` Tabs navigator, so the `if (!clientId) return` guard was preventing GET /api/categories calls from ever firing. Fixed by composing `useLocalSearchParams() ?? useGlobalSearchParams()` with Array handling in both `/app/frontend/app/admin/[clientId]/categories.tsx` and `/app/frontend/app/admin/[clientId]/manage.tsx`. (3) **Manage tab UX fixes** — manage.tsx now (a) refreshes category list alongside documents on pull-to-refresh, (b) uses DYNAMIC per-client `cats` in the Edit-Document modal instead of static CATEGORY_LABELS so admin's custom categories appear, (c) listens to `category:created/updated/deleted` WebSocket events for real-time sync. Verified end-to-end: Categories screen + Manage tab filter chips both now render all 4 custom categories correctly on mobile."
